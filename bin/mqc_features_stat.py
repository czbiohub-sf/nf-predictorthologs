...
mqc_main = """#id: 'orthology type-gs'
#plot_type: 'generalstats'
#pconfig:"""

mqc_pconf="""#    percent_{ft}:
#        title: '% {ft}'
#        namespace: 'orthology type Counts'
...
def mqc_feature_stat(bfile, features, outfile, sname=None):
    ...
    # Try to parse and read orthocount file
    fcounts = {}
    try:
        ...
        logger.error("Trouble reading the orthocount file {}".format(bfile))
        return

    total_count = sum(fcounts.values())
    if total_count == 0:
        logger.error("No orthology counts found, exiting")
        return
    ...
    if len(fpercent) == 0:
        logger.error("Any of given features '{}' not found in the orthocount file".format(", ".join(features), bfile))
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="""Calculate features percentage for orthology type counts""")
    parser.add_argument("orthocount", type=str, help="File with all orthology counts")
    ...
    parser.add_argument("-o", "--output", dest='output', default='orthocount_percent.tsv', type=str, help="Sample Name")
    args = parser.parse_args()
    mqc_feature_stat(args.orthocount, args.features, args.output, args.sample)

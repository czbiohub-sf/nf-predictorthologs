...
do_diamond_search = params.protein_searcher == "diamond" && (params.diamond_database || (raw_search_reference && params.taxonmap_gz && params.taxdmp_zip) )
...
if (params.featurecounts_hashes) {
  ...
  process bioawk_read_ids_with_hash {
    ...
    bioawk -c fastx '{ print \$name }' ${seqs_with_hashes} \\
      | awk ' { print "^" \$0 "\\s+" } '> ${read_ids_with_hashes}
    bioawk -c fastx '{ print \$name" "\$comment }' ${seqs_with_hashes} \\
      > ${read_headers_with_hashes}
    """
  }
